### Updated query_aws_hedera_gemini.py
```python
...
# CONFIG -------------------------------------------------------------------
# Hedera topic on testnet containing your sensor-data hashes\TOPIC_ID = "0.0.6025735"
...
def ask_openai(messages: list[dict], model: str = MODEL) -> str:
    """Send the conversation to OpenAI's ChatCompletion endpoint."""
...
**Next steps:**
1. Commit these changes to your GitHub repo (`streamlit_app.py` and `query_aws_hedera_gemini.py`).
2. Push to your `main` branch, then Streamlit Cloud will redeploy.
